...
class DeveloperHomeScreen extends ConsumerStatefulWidget {
  const DeveloperHomeScreen({super.key});

  @override
  ConsumerState<DeveloperHomeScreen> createState() => _DeveloperHomeScreenState();
}

class _DeveloperHomeScreenState extends ConsumerState<DeveloperHomeScreen> {
  ...
  String _tierLabel(AppLocalizations l10n, LicenseTier tier) => switch (tier) {
            ...
            children: [
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: LicenseTier.values.map((tier) {
                          return ChoiceChip(
                            label: Text(_tierLabel(l10n, tier)),
                            selected: _tier == tier,
              ...
              ..._recent.map((code) => Card(
                    child: ListTile(
                      ...
                      subtitle: Text(
                        '${code.storeNameRef} · ${_tierLabel(l10n, code.tier)} · ${code.status.name}',
                      ),
                      trailing: Text(
                        code.expiresAt == null
                            ? l10n.activationLifetime
                            : AppDateFormat.shortDate(code.expiresAt!),
                      ),
                    ),
                  )),
            ],
